...
class Atomic {
  constructor(config = {}) {
    ...
    this.packageManager = new PackageManager(this.config.packages.path, this.config.packages);
    
    this.setupCoreRoutes();
  }

  normalizeHandlerResponse(response) {
  ...
  async handleCreate(resourceType, req) {
    ...
    if (resourceDef.handlers?.create) {
      try {
        const response = await resourceDef.handlers.create.call(this, req, { 
  ...
  async handleRead(resourceType, id, req) {
    ...
    if (resourceDef.handlers?.read) {
      try {
        const response = await resourceDef.handlers.read.call(this, id, req, { 
          storage: this.storage,
          hooks: this.hooks,
          validator: this.validator,
          config: this.config
  ...
  async handleUpdate(resourceType, id, req) {
    ...
    if (resourceDef.handlers?.update) {
      try {
        const response = await resourceDef.handlers.update.call(this, id, req, { 
          storage: this.storage,
          hooks: this.hooks,
          validator: this.validator,
          config: this.config
  ...
  async handleDelete(resourceType, id, req) {
    ...
    if (resourceDef.handlers?.delete) {
      try {
        const response = await resourceDef.handlers.delete.call(this, id, req, { 
          storage: this.storage,
          hooks: this.hooks,
          validator: this.validator,
          config: this.config
  ...
  async handleSearch(resourceType, req) {
    ...
    if (resourceDef.handlers?.search) {
      try {
        const response = await resourceDef.handlers.search.call(this, req, { 
          storage: this.storage,
          hooks: this.hooks,
          validator: this.validator,
          config: this.config
