...
function calculateMatchScore(searchPatient: any, candidatePatient: any): number {
  let score = 0;
  let maxScore = 0;
...
export default defineOperation({
  ...
  async handler(params: any, context: HandlerContext): Promise<any> {
    ...
    const candidatesResult = await context.storage.search('Patient', {
      name: resource.name,
      birthDate: resource.birthDate,
      identifier: resource.identifier
    });
    const candidates = candidatesResult.entry?.map((e: any) => e.resource) || [];
